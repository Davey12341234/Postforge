...
### Build warnings (addressed in repo)

- Next.js 16: **`src/middleware.ts`** was replaced by **`src/proxy.ts`** (export `proxy`; same matcher and `/dashboard` gate).
- **`/unified`** (+ nested routes): `themeColor` moved to **`export const viewport`** in `src/app/unified/layout.tsx` (per [`generate-viewport`](https://nextjs.org/docs/app/api-reference/functions/generate-viewport)).
...
| Item | Status |
|------|--------|
| **`prisma/migrations/`** | **Present** — `init_unified_studio`, `add_unified_generated_images`, `openai_chat_and_gpt_image` (+ `migration_lock.toml`, PostgreSQL). |
...
- **Main UI:** `src/app/unified/unified-studio-client.tsx` — tabs: **Home · Create · Drafts · Publish · Stats**.  
  - **Create** includes: one-shot generate (`/api/unified/generate`), chat (`/api/unified/chat` — Anthropic or **OpenAI Responses + Conversations** via `provider` / `UNIFIED_CHAT_PROVIDER`), DALL·E 3 + **GPT Image edit**, **Whisper** transcription → chat input, preferences (`/api/unified/settings`). **No separate top-level Chat or Settings tab.**
...
| Route | Role |
|-------|------|
| `chat` | POST — **Anthropic** Messages *or* **OpenAI** Responses + Conversations (`OPENAI_API_KEY`, optional `UNIFIED_OPENAI_CHAT_MODEL`); body `provider`, `resetOpenAiConversation` |
| `generate` | POST — one-shot generation (Anthropic) |
| **`images/generate`** | **POST/GET** — **DALL·E 3** (`OPENAI_API_KEY`), see [`IMAGE-GENERATION-SPEC.md`](./IMAGE-GENERATION-SPEC.md) |
| **`images/edit`** | **POST** — **GPT Image** edit (`multipart`: image, prompt, optional mask); credits + persist like generate |
| **`audio/transcribe`** | **POST** — **Whisper** (default `whisper-1`), `multipart` `file` → text (moderated); debits unified credits |
| `credits` | GET/POST |
...
- **`ANTHROPIC_API_KEY`** — required if chat/generate use Anthropic (default chat provider).
- **`OPENAI_API_KEY`** — required for DALL·E 3, GPT Image edit, Whisper transcribe, and **OpenAI** chat path.
- Optional: **`UNIFIED_CHAT_PROVIDER`** (`anthropic` \| `openai`), **`UNIFIED_OPENAI_CHAT_MODEL`**
...
1. Confirm **`migrate deploy`** on a **fresh** Postgres (or baseline existing DB per [`PRISMA-MIGRATIONS.md`](./PRISMA-MIGRATIONS.md)). If **`npx prisma generate`** hits Windows `EPERM` on the query engine, retry after closing processes locking `node_modules\.prisma\client\`.
2. Re-run **`npm run build`** (or `npx next build` if generate is blocked) and **`npm run deploy:check`** with production-like env (`deploy/secrets.preview.env`). Optional: **`npm run verify:deploy https://your-url`**
...
5. **Smoke-test** `/unified`: auth, generate, chat (both providers if configured), image generate/edit, transcription, drafts, stats, checkout (test mode first).
6. Optional: add **Sentry**, webhook **idempotency** review.

---

## 10. One-line summary

**Unified Studio builds cleanly (use `src/proxy.ts` + `viewport` for Next.js 16); Prisma migrations include OpenAI chat + GPT Image enum; Railway uses **`GET /api/health/live`**; **`GET /api/health`** checks the database. Production deploy, DNS, and Stripe live flows remain unverified per environment.**
